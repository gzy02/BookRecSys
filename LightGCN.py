...
class LightGCN(nn.Module):
    ...
    def __init_weight(self):
        self.embedding_user = torch.nn.Embedding(num_embeddings=self.n_user,
                                                 embedding_dim=self.emb_size)
        self.embedding_item = torch.nn.Embedding(num_embeddings=self.n_item,
                                                 embedding_dim=self.emb_size)
    ...
    def getEmbedding(self, users, pos_items, neg_items, drop_flag=False):
        ...
        users_emb_ego = self.embedding_user(
            torch.tensor(users).to(self.device))
        pos_emb_ego = self.embedding_item(
            torch.tensor(pos_items).to(self.device))
        neg_emb_ego = self.embedding_item(
    ...
    def create_bpr_loss(self, users, pos, neg, drop_flag=False):
        (users_emb, pos_emb, neg_emb, userEmb0, posEmb0,
         negEmb0) = self.getEmbedding(users, pos, neg, drop_flag)
        ...
        loss = torch.mean(torch.nn.functional.softplus(neg_scores -
                                                       pos_scores))
    ...
    def create_bce_loss(self, users, pos, neg, drop_flag=False):
        ...
        pos_scores = torch.sum(torch.mul(users_emb, pos_emb), dim=1)
        neg_scores = torch.sum(torch.mul(
            users_emb.view(batch_size, 1,
                           dim).repeat(1, neg_num,
                                       1).view(batch_size * neg_num, dim),
            neg_emb),
                               dim=1)
        # neg_scores = torch.sum(torch.mul(users_emb, neg_emb), dim=1)
